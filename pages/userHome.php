...
include( "connectDb.php");
$user_id=$_SESSION['id'];
if(isset($_POST["yayinla"]) || isset($_POST["new_categori"]) || isset($_POST["new_not"]) || isset($_POST["new_header"]))
{
    //isset($_POST["new_categori"]) || isset($_POST["new_not"]) || isset($_POST["new_header"])
    $user_categori = $_POST["new_categori"];
    $user_not=$_POST["new_not"];
    $user_header=$_POST["new_header"];

    //$user_header = str_replace("\n", " ", $user_header);
    //$user_header = str_replace("&npsb", " ", $user_header);
    //$user_header = mb_strtoupper($user_header,"UTF-8");
   // mb_convert_encoding( $user_header,"utf8-turkish-ci");
    ...
    <div class="card border-0 mt-5 ms-3" >
      <!-- bu kısım not yayınlama -->
                <form class="text" method="post">
                    <label class='form-label mt-3 text-monospace' style='font-size: 20px;font-family:Century Gothic,Franklin Gothic Medium,Times New Roman;'>BAŞLIK</label> <br>                <!--  <input type="text"  style="border-width:medium;" name="new_header" pattern="[a-zA-Z]" placeholder="Başlık"><br>  -->
                      <input type="text"  style="border-width:medium;" name="new_header" placeholder="Başlık"><br> 
                     <!-- <textarea style="border-width:medium;"class="rounded scrollabletextbox border-success" name="new_header" placeholder="Başlık tek kelime olmalı!"></textarea><br> -->
                    ...
                    <button type="submit" name="yayinla"class="btn btn-outline-primary mt-2">YAYINLA</button>
                </form>
            </div>
            </div>
            <div class=" col-sm-6">
            ...
            <div class="card col-sm-6 border-0 mt-5 ms-3" >
                <form class="text" method="post">
                    ...
                    while ($data=mysqli_fetch_array($result)){
                        ...
                        echo "<option value=$noteid>$noteid</option>";
                 }
                 echo "</select><br>";
                 if(isset($_POST["categories"]))
                 {
                  $header=$_POST["categories"];
                  //mb_convert_encoding( $header,"utf8");
                 }
                 ...
                 echo "
                    ...
                    if(isset($_POST["updateNote"])|| isset($_POST["updatingNote"]))
                    {
                      ...
                      $sql="UPDATE note SET content ='".$updateText."' WHERE id='$header'";
                    ...
                    if(isset($_POST["deleteNote"]))
                    {
                      ...
                      $sql="DELETE FROM note WHERE id='$header'";
                      //$sql="UPDATE note SET 'releaseNote' WHERE contentHeader='$header'";
                      $cevap=mysqli_query($connection,$sql);
                    }
                echo"    
                     ...
                     $header = mb_strtoupper($header,"UTF-8");
                          ...
                          $noteid=$data['id'];
                            echo "
                                ...
                                <p class='fs-5'>$header</p>
                                <p class='fw-light'>$yazarismi tarafından $tarih tarihinde yazıldı.  Not kodu: <strong>$noteid</strong></p><hr>
                                <p class='fst-normal'>$text</p>
                                </div>";
